# Studio Baggio — Homepage copy
*Locked draft. 26 May 2026.*

Throughline: *argument → gap → what we build → proof → offer → promise → fit → CTA*.

## Open actions (add when available)
- **Client quotes — in progress this week (w/c 26 May 2026).** Targets: Charlotte Evans (leadership work), Luke Goody (Hanbury internal advocate), Harry Sims (Calm Authority co-founder), Joe Churchill (FanClub). Hanbury itself still aspirational. Slot into Section 5 (Proof) or directly under the hero as social proof.
- **Lead magnet — planned, not yet built.** AI diagnostic in the shape of Calm Authority's visibility diagnostic, but for Studio Baggio's offering (where AI can create value in the buyer's business). Sits above the "Discuss your AI opportunity" CTA as a lower-friction first step. Build after homepage copy is locked.
- **Offer structure — to revisit after client conversations this week.** Hanbury proposal is the full offer; productised smaller elements (AI Opportunity Audit, individual service modules) need scoping. Tied to: price/scope signal below, lead magnet above, and contact method below.
- **First-contact mechanism — call vs email form.** Currently "Discuss your AI opportunity" implies a call. Lower-friction email form likely converts better (per Hormozi take), but the lead magnet would handle that role. Decide once offer structure is clearer.
- **Price / scope signal for AI Opportunity Audit** — add to Section 6 once decided. Format: "Engagements start at £X" or "Two-week diagnostic" or similar. Helps prospects self-qualify before booking.
- **Closing CTA pull line** — Section 10 currently button-only. Add a one-line nudge above the button when written.
...
## Section 2 — Opening argument

Eyebrow: `FOR COMPANIES THAT WIN ON TRUST AND CREDIBILITY`

H1: `Most businesses are using AI. Few are using it to create commercial advantage.`

Sub: `Studio Baggio designs and builds the AI systems that make expert-led businesses:`
