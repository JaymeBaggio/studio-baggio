...
type ProductWorkItem = WorkItem & { productPage: ProductPageData };
  ...
  return (
    <div className="home-4b studio-page products-page">
      ...
      <AnimatePresence mode="wait" initial={false}>
        <ProductPanel key={activeItem.slug} item={activeItem} />
      </AnimatePresence>
    </div>
  );
}

function ProductPanel({ item }: { item: ProductWorkItem }) {
  ...
  return (
    ...
    >
      <header className="editorial-container products-panel-header">
        ...
        <h2 className="products-panel-title">{item.title}</h2>
      </header>
      ...
      <div className="editorial-container products-panel-main">
        <div className={panelGridClassName}>
          ...
          <aside className="products-panel-aside">
            <figure className="products-panel-media">
              <Image
          ...
          <div className="editorial-container products-featured-inner">
            ...
            <div>
              {productPage.featuredIn.map((publication) => (
                <span key={publication}>{publication}</span>
              ))}
            </div>
          </div>
        </div>
