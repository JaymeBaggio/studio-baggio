...
- **Second pass, same day, after "update the site PROPERLY":** every blue full stop title device across the site is now the square (`.sb-mark`): services hero, about, contact, products showcase, research index, research masthead, the three research report titles and their OG images, and the 90-day article closing line. Old dot CSS rules removed. `public/downloads/studio-baggio-introduction.pdf` re-rendered from the intro source with the new wordmark. Commercial PDFs re-rendered too (intro, finance case-study partnership SEP2026, HNW/UHNW SEP2026; old versions archived beside them). LinkedIn avatars recut so the B and mark clear the circular mask.
- **Third pass, same day, after she found the press title still a dot:** full audit of all 32 sitemap URLs in her Chrome (text dots, round shapes, and pseudo-element dots) plus a source sweep. Fixed the three that the class-name sweep had missed: press hero title, insights hero title (both unclassed spans styled round in CSS) and the insight-article hero `::after` dot on the setup-guide and ninety-days article classes. All 32 pages audited clean on the preview before push. Deliberately left: blue list bullets in articles and services, and the status light in the SEO audit pill; those are UI marks, not the title device.
...
- **Fire Source live-site link removed 20 Sept (b0cf966):** no "Visit live site" on the homepage card or the products page; "Find out more" only, same as Business Tracker. Jayme: "firesource doesn't work well". Verified live in Chrome: zero fire-source anchors on / and /work.
- **/llms.txt LIVE 20 Sept:** generated route `src/app/llms.txt/route.ts` built from the same content files as the sitemap (insights with month/year + meta description, research editions, the two law benchmarks, company pages, products, PDFs). Cache 1h/24h CDN. Article audit the same day: 2,782 words server-rendered, BlogPosting + FAQPage + BreadcrumbList schema, 1 H1 / 3 H2 / 19 H3, canonical and OG present, robots open.
...
- Added Jayme's approved visible speaker copy to the existing `Press & Founder Availability` block on `/about`.
...
- Replaced the stale mission and two-route framework in `What we do` with the Services-page proposition and four current offers. Each offer links to its exact service section.
...
- Replaced the briefly published audit-led homepage preview with a company-level 1200×675 Studio Baggio wordmark card. The audit artwork remains a separate asset and its misplaced blue dot was corrected.
- Selected copy: `FIND WHERE AI CREATES MEASURABLE VALUE.` followed by `BUILD THE SYSTEMS NEEDED TO DELIVER IT.`
- Restored the company title `Studio Baggio | AI Strategy, Systems & Implementation` and aligned the Open Graph/Twitter description to the same measurable-value proposition.
- Published the main-site artwork at the new versioned `studio-baggio-og-v5.png` URL. Typecheck, lint, `git diff --check` and the production build passed; commit `754de2c`, custom-domain metadata, WhatsApp crawler response, 1200×675 dimensions and live image bytes were verified.
